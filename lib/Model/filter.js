  ...
  function filterListener(segments, eventArgs) {
    ...
    for (var path in map) {
      ...
      if (util.mayImpact(filter.inputSegments, segments)) {
        // Deal with special cases when filter is ref'd:
        // On changes, the filter is ref'd and a whole element is replaced
        // - then check if the element existed previously or if there is 
        // no filter, then we need to postpone any changes because a 
        // potential resorting (or an odd case where Derby-triggering will
        // affect the wrong element) will make things happen in the wrong 
        // order. Basically, if resorted, the actual move will happen before 
        // any other listeners will have had time to rely on the model being 
        // untouched and thus will potentially use incorrect data due to 
        // incorrect assumptions about the model state. In addition, this 
        // issue only arises in the browser shim version of the event 
        // emitter thus only do it there (where browser-builtins is used).
        if(!util.isServer && eventArgs[0] === 'change' && filter.from && segments.length - 1 === filter.inputSegments.length) {
          var changeKey = segments[segments.length - 1];
          var index = filter.model._get(filter.idsSegments).indexOf(changeKey);
          var res = (!filter.filterFn || index > -1);

          // We know the event will propagate to through the refList
          // because the element existed, thus, let's postpone the filter
          // event to avoid issues with resorting to the ref'd element.
          if(res) {
            var listener = model.on('change', filter.fromSegments.concat(['**']).join('.'), change(index, filter));
            return;
          }
        }
...
Model.prototype.filter = function(input, fn, ctx, skip, limit) {
  var inputPath = this.path(input);
  return this._filters.add(inputPath, fn, null, ctx, skip, limit);
};

Model.prototype.sort = function(input, fn, ctx, skip, limit) {
  var inputPath = this.path(input);
  return this._filters.add(inputPath, null, fn || 'asc', ctx, skip, limit);
};
...
Filter.prototype.ids = function() {
  ...
  var ids = [];
  var ctx = typeof this.ctx === 'object' && this.ctx !== null ? util.copyObject(this.ctx) : this.ctx;
  if (!items) return ids;
